...
- [x] **HDF5 Support**: Implement binary format support for large datasets (currently only JSONL is supported).
...
- [x] **FastAPI Bridge**: Expose full `World` state in Production mode via REST/WebSocket.
- [ ] **Telemetry Dashboard**: Enhance `TelemetrySystem` to support Prometheus/Grafana export.
- [x] **Mode Configuration Files**: Allow defining mode parameters (like system exclusions) in JSON/YAML config files instead of hardcoded Python classes.
- [x] **Gymnasium Reset Logic**: Implement efficient environment reset without full engine restart.
